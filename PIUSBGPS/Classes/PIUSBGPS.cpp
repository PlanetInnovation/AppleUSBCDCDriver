 ...
 */
    ...
    /* PIUSBGPS.cpp - MacOSX implementation of		*/
...
#define DEBUG_NAME "PIUSBGPS"

#include "PIUSBGPSCommon.h"
#include "PIUSBGPS.h"
#include "PIUSBGPSPrivate.h"
...
OSDefineMetaClassAndStructors(PIUSBGPS, IOService);

/****************************************************************************************************/
//
//		Function:	PIUSBGPS::Asciihex_to_binary
//
...
UInt8 PIUSBGPS::Asciihex_to_binary(char c)
{
...
bool PIUSBGPS::start(IOService *provider)
{
...
void PIUSBGPS::stop(IOService *provider)
{
...
IOReturn PIUSBGPS::reInitDevice()
{
	IOUSBDevRequest req;
    IOReturn		ior = kIOReturnSuccess;
...
//                            Log("PIUSBGPS: checkECM - Ethernet address[%d] = %8x\n",(unsigned int)(i),(unsigned int)(fCacheEaddr[i]));
                        }
                ...
                else if (funcDesc->bDescriptorSubtype == MBIM_Functional_Descriptor) //rcs!!! 2 /4/2013
                {
                    ...
                    if (ior == kIOReturnSuccess)
                    {
                        for (i = 0; i < 6; i++)
                        {
                            fCacheEaddr[i] = (Asciihex_to_binary(ascii_mac[i*2]) << 4) | Asciihex_to_binary(ascii_mac[i*2+1]);
                            // Log("PIUSBGPS: checkECM - Ethernet address[%d] = %8x\n",(unsigned int)(i),(unsigned int)(fCacheEaddr[i]));
                            kprintf("PIUSBGPS: checkECM - Ethernet address[%d] = %8x\n",(unsigned int)(i),(unsigned int)(fCacheEaddr[i]));
                        }
                        XTRACE(this, 0, addrString, "checkECM - Ethernet address (cached)");
                    } else {
                        XTRACE(this, ior, addrString, "checkECM - Error retrieving Ethernet address");
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPS::confirmControl
//
...
IOReturn PIUSBGPS::message(UInt32 type, IOService *provider, void *argument)
{	
    ...
    switch (type)
    {
        ...
        case kIOMessageServiceWasClosed: 	
            #if LDEBUG
            OSReportWithBacktrace("PIUSBGPS::message called with kIOMessageServiceWasClosed ");
            #endif
        ...
        default:
        #if LDEBUG
            OSReportWithBacktrace("PIUSBGPS::message called with Unknown type %lx ",type);
        #endif
...
IOCommandGate *PIUSBGPS::getCommandGate() const
{
    return fCommandGate;
}


IOReturn PIUSBGPS::setProperties( OSObject * properties )
{
...
IOReturn PIUSBGPS::setPropertiesAction(	OSObject	*owner, 
														void		*arg1, 
														void		*arg2, 
														void		*arg3, 
														void		*arg4 )
{
	...
	if ( owner != NULL )
	{
		PIUSBGPS *me = OSDynamicCast( PIUSBGPS, owner );
...
IOReturn PIUSBGPS::setPropertiesWL( OSObject * properties )
{
	IOReturn result = kIOReturnBadArgument;
	...
	if ( propertyDict != NULL )
	{
		...
		{
			Log("PIUSBGPS::setProperties - SuspendDevice: true\n");
			
			if ( fpDevice->isOpen(this) )
			{
				if ( ( result = fpDevice->SuspendDevice(true) ) != kIOReturnSuccess )
				{
					Log("PIUSBGPS::setProperties - failed to suspend the device, error: %08x \n", result);
				}
			}
			else
			{
				Log("PIUSBGPS::setProperties - device was not open \n");
		...
		{
			Log("PIUSBGPS::setProperties - SuspendDevice: false\n");
			...
			{
				IOLog("PIUSBGPS::setProperties - device was not open \n");
		...
		if ( remoteWakeup )
		{
			...
			{
				IOLog("PIUSBGPS::setProperties - Set/Clear remote wake up feature successful\n");
			} else {
				IOLog("PIUSBGPS::setProperties - Set/Clear remote wake up feature failed, %08x\n", result);
		}
