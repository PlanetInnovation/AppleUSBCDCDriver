...
#include "PIUSBGPSACMDataUser.h"

#define baseName		"PI-GPS"
#define hiddenTag		"HiddenPort"
#define WWANTag			"WWAN"

#define defaultName		"PI GPS"
...
class PIUSBGPS;
class PIUSBGPSACMControl;

	/* PIUSBGPSACMData.h - This file contains the class definition for the		*/
	/* USB Communication Device Class (CDC) Data Interface driver - ACM only at the moment.	*/

class PIUSBGPSACMData : public IOSerialDriverSync
{
    OSDeclareDefaultStructors(PIUSBGPSACMData);			// Constructor & Destructor stuff

private:
	PIUSBGPS		*fCDCDriver;			// The CDC driver
	PIUSBGPSACMControl   *fControlDriver;			// Our Control Driver
...
public:
    ...
    static bool				bsdClientPublished(PIUSBGPSACMData *target, void *ref, IOService *newService,IONotifier * notifier);
...
}; /* end class PIUSBGPSACMData */

class PIUSBGPSACMData;

class PIUSBGPSACMDataUserClient : public IOUserClient
{
    OSDeclareDefaultStructors(PIUSBGPSACMDataUserClient);

private:
    PIUSBGPSACMData	*fProvider;
...
}; /* end class PIUSBGPSACMDataUserClient */
#endif