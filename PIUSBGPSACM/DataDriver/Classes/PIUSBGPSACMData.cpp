 ...
 */
    ...
    /* PIUSBGPSACMData.cpp - MacOSX implementation of		*/
...
#define DEBUG_NAME "PIUSBGPSACMData"

#include "PIUSBGPSACM.h"
#include "PIUSBGPSACMData.h"

#define MIN_BAUD (50 << 1)

//PIUSBGPSACMControl		*gControlDriver = NULL;			// Our Control driver

static const OSSymbol *gPMWakeOnRingSymbol = NULL;

#define super IOSerialDriverSync

OSDefineMetaClassAndStructors(PIUSBGPSACMData, IOSerialDriverSync);
...
PIUSBGPS *findCDCDriverAD(void *dataAddr, UInt8 dataInterfaceNum, IOReturn *retCode)
{
    PIUSBGPSACMData	*me = (PIUSBGPSACMData *)dataAddr;
    PIUSBGPS		*CDCDriver = NULL;
    bool		driverOK = false;
    ...
    matchingDictionary = IOService::serviceMatching("PIUSBGPS");
    ...
    if (!iterator)
    {
        XTRACE(me, 0, 0, "findCDCDriverAD - No PIUSBGPS driver found!");
    ...
    CDCDriver = (PIUSBGPS *)iterator->getNextObject();
    while (CDCDriver)
    {
...
//
//		Outputs:	PIUSBGPSACMControl
//
//		Desc:		Finds our matching control driver
//
/****************************************************************************************************/

PIUSBGPSACMControl *findControlDriverAD(void *me)
{
    Boolean                      worked             = false;
    PIUSBGPSACMControl	*tempDriver         = NULL;
    OSIterator			*iterator           = NULL;
    OSDictionary		*matchingDictionary = NULL;
    ...
    matchingDictionary = IOService::serviceMatching("PIUSBGPSACMControl");
    if (!matchingDictionary)
    {
    ...
    if (!iterator)
    {
        XTRACE(me, 0, 0, "findControlDriverAD - No PIUSBGPSACMControl drivers found (iterator)");
        matchingDictionary->release();
        return NULL;
    }
    ...
    tempDriver = (PIUSBGPSACMControl *)iterator->getNextObject();
    while (tempDriver)
    {
        XTRACEP(me, 0, tempDriver, "findControlDriverAD - Data driver candidate");
        if (tempDriver->checkInterfaceNumber((PIUSBGPSACMData *)me))
        {
...
void PIUSBGPSACMData::USBLogData(UInt8 Dir, SInt32 Count, char *buf)
{    
    SInt32	wlen;
    SInt32	llen, rlen;
    ...
    {
        case kDataIn:
            Log( "PIUSBGPSACMData: USBLogData - Read Complete, address = %8p, size = %8d\n", (void *)buf, (UInt)Count );
            break;
        case kDataOut:
            Log( "PIUSBGPSACMData: USBLogData - Write, address = %8p, size = %8d\n", (void *)buf, (UInt)Count );
            break;
        case kDataOther:
            Log( "PIUSBGPSACMData: USBLogData - Other, address = %8p, size = %8d\n", (void *)buf, (UInt)Count );
            break;
    }
    ...
    if (wlen == 0)
    {
        Log( "PIUSBGPSACMData: USBLogData - No data, Count=0\n" );
        return;
    }
...
/****************************************************************************************************/
//
//		Function:	PIUSBGPSACMData::dumpData
//
//		Inputs:		Dir - direction
//					buf - the data
...
void PIUSBGPSACMData::dumpData(UInt8 Dir, char *buf, SInt32 Count)
{
    ...
    {
        case kDataIn:
            Log( "PIUSBGPSACMData: dumpData - Read Complete, address = %8p, size = %8d\n", (void *)buf, (UInt)Count );
            break;
        case kDataOut:
            Log( "PIUSBGPSACMData: dumpData - Write, address = %8p, size = %8d\n", (void *)buf, (UInt)Count );
            break;
        case kDataOther:
            Log( "PIUSBGPSACMData: dumpData - Other, address = %8p, size = %8d\n", (void *)buf, (UInt)Count );
            break;
    }
...
QueueStatus PIUSBGPSACMData::AddBytetoQueue(CirQueue *Queue, char Value)
{
...
QueueStatus PIUSBGPSACMData::GetBytetoQueue(CirQueue *Queue, UInt8 *Value)
{
...
QueueStatus PIUSBGPSACMData::InitQueue(CirQueue *Queue, UInt8 *Buffer, size_t Size)
{
    Queue->Start	= Buffer;
...
QueueStatus PIUSBGPSACMData::CloseQueue(CirQueue *Queue)
{
...
/****************************************************************************************************/
//
//		Function:	PIUSBGPSACMData::AddtoRXQueue
//
//		Inputs:		Queue - the queue to be added to
//					buffs - data to add
//					Size - length of data
...
size_t PIUSBGPSACMData::AddtoRXQueue(CirQueue *Queue, inPipeBuffers *buffs, size_t Size)
{
...
size_t PIUSBGPSACMData::AddtoQueue(CirQueue *Queue, UInt8 *Buffer, size_t Size)
{
    size_t	BytesWritten = 0;
...
size_t PIUSBGPSACMData::RemovefromQueue(CirQueue *Queue, UInt8 *Buffer, size_t MaxSize)
{
...
size_t PIUSBGPSACMData::FreeSpaceinQueue(CirQueue *Queue)
{
    size_t	retVal = 0;
...
size_t PIUSBGPSACMData::UsedSpaceinQueue(CirQueue *Queue)
{
...
size_t PIUSBGPSACMData::GetQueueSize(CirQueue *Queue)
{
    return Queue->Size;
...
QueueStatus PIUSBGPSACMData::GetQueueStatus(CirQueue *Queue)
{
    if ((Queue->NextChar == Queue->LastChar) && Queue->InQueue)
        return queueFull;
...
void PIUSBGPSACMData::CheckQueues()
{
    UInt32	Used;
    UInt32	Free;
    UInt32	QueuingState;
...
void PIUSBGPSACMData::CheckHold()
{
	SInt32			size;
	inPipeBuffers	*buffs;
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::dataReadComplete
//
//		Inputs:		obj - me
...
void PIUSBGPSACMData::dataReadComplete(void *obj, void *param, IOReturn rc, UInt32 remaining)
{
    PIUSBGPSACMData	*me = (PIUSBGPSACMData*)obj;
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::dataWriteComplete
//
//		Inputs:		obj - me
//				param - the buffer pool pointer
//				rc - return code
...
void PIUSBGPSACMData::dataWriteComplete(void *obj, void *param, IOReturn rc, UInt32 remaining)
{
    PIUSBGPSACMData	*me = (PIUSBGPSACMData *)obj;
...
IOService* PIUSBGPSACMData::probe( IOService *provider, SInt32 *score )
{ 
    IOService   *res;
...
bool PIUSBGPSACMData::start(IOService *provider)
{
	IOReturn	rtn;
    ...
    cdc_LogToMessageTracer(CDC_ASL_DOMAIN, "PIUSBGPSACMData", vendorString, productString, 0, 0);
...
void PIUSBGPSACMData::stop(IOService *provider)
{
    IOReturn	ret;
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::stopAction
//
//		Desc:		Dummy pass through for stopGated.
//
/****************************************************************************************************/

IOReturn PIUSBGPSACMData::stopAction(OSObject *owner, void *, void *, void *, void *)
{

    ((PIUSBGPSACMData *)owner)->stopGated();
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::stopGated
//
//		Inputs:		
//
//		Outputs:	
...
void PIUSBGPSACMData::stopGated()
{
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::createSuffix
//
//		Inputs:		
//
...
bool PIUSBGPSACMData::createSuffix(unsigned char *sufKey)
{
...
bool PIUSBGPSACMData::findSerialBSDClient (IOModemSerialStreamSync *nub) 
{
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::waitForBSDClienAction
//
//		Desc:		Dummy pass through for sendDeviceRequestGated
//
/****************************************************************************************************/

IOReturn PIUSBGPSACMData::waitForBSDClienAction(OSObject *owner, void *, void *, void *, void *)
{
    return ((PIUSBGPSACMData *)owner)->waitForBSDClientGated();
}   // end sendDeviceRequestAction


/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::waitForBSDClientGated
//
//		Inputs:		
//
...
IOReturn PIUSBGPSACMData::waitForBSDClientGated()
{
...
bool PIUSBGPSACMData::bsdClientPublished (PIUSBGPSACMData * target, void * ref, IOService * newService, IONotifier * notifier) 
{
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::createSerialStream
//
//		Inputs:		
//
...
bool PIUSBGPSACMData::createSerialStream()
{
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::acquirePort
//
...
IOReturn PIUSBGPSACMData::acquirePort(bool sleep, void *refCon)
{
    IOReturn                ret;
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::acquirePortAction
//
//		Desc:		Dummy pass through for acquirePortGated.
//
/****************************************************************************************************/

IOReturn PIUSBGPSACMData::acquirePortAction(OSObject *owner, void *arg0, void *, void *, void *)
{

    return ((PIUSBGPSACMData *)owner)->acquirePortGated((bool)arg0);
    
}/* end acquirePortAction */

/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::acquirePortGated
//
//		Inputs:		sleep - true (wait for it), false (don't)
//
//		Outputs:	Return Code - kIOReturnSuccess, kIOReturnExclusiveAccess, kIOReturnIOError and various others
...
IOReturn PIUSBGPSACMData::acquirePortGated(bool sleep)
{
    UInt32 	busyState = 0;
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::releasePort
//
...
IOReturn PIUSBGPSACMData::releasePort(void *refCon)
{
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::releasePortAction
//
//		Desc:		Dummy pass through for releasePortGated.
//
/****************************************************************************************************/

IOReturn PIUSBGPSACMData::releasePortAction(OSObject *owner, void *, void *, void *, void *)
{

    return ((PIUSBGPSACMData *)owner)->releasePortGated();
    
}/* end releasePortAction */

/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::releasePortGated
//
//		Inputs:		
//
//		Outputs:	Return Code - kIOReturnSuccess or kIOReturnNotOpen
...
IOReturn PIUSBGPSACMData::releasePortGated()
{
    UInt32 	busyState;
	UInt32	state;
	UInt32	mask;
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::getState
//
//		Inputs:		refCon - unused
//
...
UInt32 PIUSBGPSACMData::getState(void *refCon)
{
    UInt32	currState;
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::getStateAction
//
//		Desc:		Dummy pass through for getStateGated.
//
/****************************************************************************************************/

IOReturn PIUSBGPSACMData::getStateAction(OSObject *owner, void *, void *, void *, void *)
{
    UInt32	newState;

    newState = ((PIUSBGPSACMData *)owner)->getStateGated();
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::getStateGated
//
//		Inputs:		port - unused
//
//		Outputs:	return value - port state
...
UInt32 PIUSBGPSACMData::getStateGated()
{
    UInt32 	state;
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::setState
//
...
IOReturn PIUSBGPSACMData::setState(UInt32 state, UInt32 mask, void *refCon)
{
    IOReturn	ret = kIOReturnSuccess;
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::setStateAction
//
//		Desc:		Dummy pass through for setStateGated.
//
/****************************************************************************************************/

IOReturn PIUSBGPSACMData::setStateAction(OSObject *owner, void *arg0, void *arg1, void *, void *)
{

    return ((PIUSBGPSACMData *)owner)->setStateGated((UInt32 *)arg0, (UInt32 *)arg1);
    
}/* end setStateAction */

/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::setStateGated
//
...
IOReturn PIUSBGPSACMData::setStateGated(UInt32 *pState, UInt32 *pMask)
{
	UInt32	state = *pState;
	UInt32	mask = *pMask;
    UInt32	delta;
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::watchState
//
...
IOReturn PIUSBGPSACMData::watchState(UInt32 *state, UInt32 mask, void *refCon)
{
    IOReturn 	ret;
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::watchStateAction
//
//		Desc:		Dummy pass through for watchStateGated.
//
/****************************************************************************************************/

IOReturn PIUSBGPSACMData::watchStateAction(OSObject *owner, void *arg0, void *arg1, void *, void *)
{

    return ((PIUSBGPSACMData *)owner)->watchStateGated((UInt32 *)arg0, (UInt32 *)arg1);
    
}/* end watchStateAction */

/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::watchStateGated
//
//		Inputs:		state - state to watch for
//				mask - state mask bits
//
...
IOReturn PIUSBGPSACMData::watchStateGated(UInt32 *pState, UInt32 *pMask)
{
	UInt32		mask = *pMask;
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::nextEvent
//
//		Inputs:		refCon - unused
//
...
UInt32 PIUSBGPSACMData::nextEvent(void *refCon)
{
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::executeEvent
//
...
IOReturn PIUSBGPSACMData::executeEvent(UInt32 event, UInt32 data, void *refCon)
{
    IOReturn 	ret;
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::executeEventAction
//
//		Desc:		Dummy pass through for executeEventGated.
//
/****************************************************************************************************/

IOReturn PIUSBGPSACMData::executeEventAction(OSObject *owner, void *arg0, void *arg1, void *, void *)
{

    return ((PIUSBGPSACMData *)owner)->executeEventGated((UInt32 *)arg0, (UInt32 *)arg1);
    
}/* end executeEventAction */

/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::executeEventGated
//
//		Inputs:		event - The event
//				data - any data associated with the event
//
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::requestEvent
//
//		Inputs:		event - The event
//				refCon - unused
//
...
IOReturn PIUSBGPSACMData::requestEvent(UInt32 event, UInt32 *data, void *refCon)
{
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::enqueueEvent
//
//		Inputs:		event - The event
//				data - any data associated with the event, 
...
IOReturn PIUSBGPSACMData::enqueueEvent(UInt32 event, UInt32 data, bool sleep, void *refCon)
{
    IOReturn 	ret;
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::dequeueEvent
//
//		Inputs:		sleep - true (wait for it), false (don't)
//				refCon - unused
//
...
IOReturn PIUSBGPSACMData::dequeueEvent(UInt32 *event, UInt32 *data, bool sleep, void *refCon)
{
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::enqueueData
//
...
IOReturn PIUSBGPSACMData::enqueueData(UInt8 *buffer, UInt32 size, UInt32 *count, bool sleep, void *refCon)
{
    IOReturn 	ret;
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::enqueueDatatAction
//
//		Desc:		Dummy pass through for enqueueDataGated.
//
/****************************************************************************************************/

IOReturn PIUSBGPSACMData::enqueueDataAction(OSObject *owner, void *arg0, void *arg1, void *arg2, void *arg3)
{

    return ((PIUSBGPSACMData *)owner)->enqueueDataGated((UInt8 *)arg0, (UInt32 *)arg1, (UInt32 *)arg2, (bool *)arg3);
    
}/* end enqueueDataAction */

/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::enqueueDataGated
//
//		Inputs:		buffer - the data
//				size - number of bytes
//				sleep - true (wait for it), false (don't)
...
IOReturn PIUSBGPSACMData::enqueueDataGated(UInt8 *buffer, UInt32 *pSize, UInt32 *count, bool *pSleep)
{
	UInt32		size = *pSize;
	bool		sleep = *pSleep;
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::dequeueData
//
...
IOReturn PIUSBGPSACMData::dequeueData(UInt8 *buffer, UInt32 size, UInt32 *count, UInt32 min, void *refCon)
{
    IOReturn 	ret;
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::dequeueDatatAction
//
//		Desc:		Dummy pass through for dequeueDataGated.
//
/****************************************************************************************************/

IOReturn PIUSBGPSACMData::dequeueDataAction(OSObject *owner, void *arg0, void *arg1, void *arg2, void *arg3)
{

    return ((PIUSBGPSACMData *)owner)->dequeueDataGated((UInt8 *)arg0, (UInt32 *)arg1, (UInt32 *)arg2, (UInt32 *)arg3);
    
}/* end dequeueDataAction */

/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::dequeueDataGated
//
//		Inputs:		size - buffer size
//				min - minimum bytes required
//
...
IOReturn PIUSBGPSACMData::dequeueDataGated(UInt8 *buffer, UInt32 *pSize, UInt32 *count, UInt32 *pMin)
{
	UInt32		size = *pSize;
	UInt32		min = *pMin;
    IOReturn 	rtn = kIOReturnSuccess;
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::setUpTransmit
//
//		Inputs:		
//
...
bool PIUSBGPSACMData::setUpTransmit()
{
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::startTransmission
//
//		Inputs:		
//
//		Outputs:	
...
void PIUSBGPSACMData::startTransmission()
{
    size_t		count;
    IOReturn	ior;
    UInt16		indx;
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::setLineCoding
//
//		Inputs:		
//
//		Outputs:	
...
void PIUSBGPSACMData::setLineCoding()
{
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::setControlLineState
//
...
void PIUSBGPSACMData::setControlLineState(bool RTS, bool DTR)
{
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::sendBreak
//
...
void PIUSBGPSACMData::sendBreak(bool sBreak)
{
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::checkPipe
//
...
IOReturn PIUSBGPSACMData::checkPipe(IOUSBPipe *thePipe, bool devReq)
{
    IOReturn 	rtn = kIOReturnSuccess;
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::initStructure
//
//		Inputs:		
//
//		Outputs:	
...
void PIUSBGPSACMData::initStructure()
{
    UInt16	i;
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::setStructureDefaults
//
//		Inputs:		
//
//		Outputs:	
...
void PIUSBGPSACMData::setStructureDefaults()
{
    UInt32	tmp;
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::allocateResources
//
//		Inputs:		
//
...
bool PIUSBGPSACMData::allocateResources()
{
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::releaseResources
//
//		Inputs:		
//
//		Outputs:	
...
void PIUSBGPSACMData::releaseResources()
{
    UInt16	i;
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::freeRingBuffer
//
...
void PIUSBGPSACMData::freeRingBuffer(CirQueue *Queue)
{
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::allocateRingBuffer
//
...
bool PIUSBGPSACMData::allocateRingBuffer(CirQueue *Queue, size_t BufferSize)
{
    UInt8	*Buffer;
...
/****************************************************************************************************/
//
//		Function:	PIUSBGPSACMData::handleSettingCallback
//
//		Inputs:		
//
//		Outputs:	none	
...
void PIUSBGPSACMData::handleSettingCallback(const OSSymbol *arg_type, OSObject *arg_val, uintptr_t refcon)
{
    UInt32				WoR;
...
/****************************************************************************************************/
//
//		Function:	PIUSBGPSACMData::setupWakeOnRingPMCallback
//
//		Inputs:		none
//
...
bool PIUSBGPSACMData::setupWakeOnRingPMCallback()
{
	IOReturn		ior;
	bool			worOK = false;
	...
	if (fPMRootDomain)
	{
		...
		ior = fPMRootDomain->registerPMSettingController(settings_arr,
														 ...
														 (OSObject*)this,
														 &PIUSBGPSACMData::handleSettingCallback),
														 (OSObject *)this,
														 (uintptr_t)NULL,
...
/****************************************************************************************************/
//
//		Function:	PIUSBGPSACMData::WakeonRing
//
//		Inputs:		none
//
...
bool PIUSBGPSACMData::WakeonRing(void)
{
...
/****************************************************************************************************/
//
//		Function:	PIUSBGPSACMData::setWakeFeature
//
//		Inputs:		none
//
//		Outputs:	none
...
void PIUSBGPSACMData::setWakeFeature(void)
{
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::clearSleepingThreads
//
//		Inputs:		
//
//		Outputs:	
...
void PIUSBGPSACMData::clearSleepingThreads()
{
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::resurrectRead
//
//		Inputs:		
//
//		Outputs:	
...
void PIUSBGPSACMData::resurrectRead()
{
    UInt16		i;
	IOReturn	rtn;
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::didTerminate
//
...
bool PIUSBGPSACMData::didTerminate(IOService *provider, IOOptionBits options, bool *defer)
{
	bool	result = false;
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::message
//
...
IOReturn PIUSBGPSACMData::message(UInt32 type, IOService *provider, void *argument)
{	
			...
			if (!fSuppressWarning)
			{
				if (fSessions)
				{
					...
					{ 
						...
						"/System/Library/Extensions/IOUSBFamily.kext/Contents/PlugIns/PIUSBGPSACMData.kext",		// localizationPath
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMData::getPortNameForInterface
//
...
OSString *PIUSBGPSACMData::getPortNameForInterface(UInt8 interfaceNumber)
{
...
OSDefineMetaClassAndStructors(PIUSBGPSACMDataUserClient, IOUserClient);

/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMDataUserClient::getTargetAndMethodForIndex
//
//		Inputs:		
//
...
//IOExternalMethod *PIUSBGPSACMDataUserClient::getExternalMethodForIndex(UInt32 index)
IOExternalMethod *PIUSBGPSACMDataUserClient::getTargetAndMethodForIndex(IOService **targetP, UInt32 index)
{
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMDataUserClient::initWithTask
//
...
bool PIUSBGPSACMDataUserClient::initWithTask(task_t owningTask, void *security_id , UInt32 type)
{
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMDataUserClient::start
//
//		Inputs:		provider - my provider
//
...
bool PIUSBGPSACMDataUserClient::start(IOService *provider)
{
    ...
    fProvider = OSDynamicCast(PIUSBGPSACMData, provider);
    if (!fProvider)
    {
    ...
    fMethods[0].func   = (IOMethod)&PIUSBGPSACMDataUserClient::doRequest;
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMDataUserClient::clientClose
//
//		Inputs:		
//
//		Outputs:	return code - kIOReturnSuccess
...
IOReturn PIUSBGPSACMDataUserClient::clientClose()
{
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMDataUserClient::clientDied
//
//		Inputs:		
//
//		Outputs:	return code - kIOReturnSuccess
...
IOReturn PIUSBGPSACMDataUserClient::clientDied()
{
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMDataUserClient::doRequest
//
...
IOReturn PIUSBGPSACMDataUserClient::doRequest(void *pIn, void *pOut, IOByteCount inputSize, IOByteCount *pOutPutSize)
{
    UInt8	*input;
...
/****************************************************************************************************/
//
//		Method:		PIUSBGPSACMDataUserClient::ACMDataMessage
//
...
IOReturn PIUSBGPSACMDataUserClient::ACMDataMessage(void *pIn, void *pOut, IOByteCount inputSize, IOByteCount *pOutPutSize)
{
